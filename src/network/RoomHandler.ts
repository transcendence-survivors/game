 ...
 *   `onChange`) into calls on the {@link PlayerRegistry}.
...
/** Minimal structural type for what the registry needs to read from a player. */
...
export class RoomHandler {
	...
	constructor(room: Room, registry: PlayerRegistry, input: InputManager, sendRateHz: number) {
		this.room = room;
	...
	attach(): void {
		...
		const state = this.room.state as { players: { onAdd: unknown; onRemove: unknown } };
