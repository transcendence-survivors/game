...
import { HealthSystem, MonsterAI, Vec3 } from "@transcendence/game-shared";
import type {
  IVec3,
  MonsterType,
  TerrainGenerator,
...
export class Monster {
  ...
  constructor(scene: Scene, terrain: TerrainGenerator, config: MonsterConfig) {
    ...
    const mesh = MeshBuilder.CreateBox(
      `monster_${config.type.id}_${config.id}`,
      {
  ...
  computeMove(playerPosition: IVec3, peerPositions: IVec3[]): IVec3 {
    return this._ai.computeMoveDirection(
      this._asVec(),
      playerPosition,
      peerPositions,
    );
  ...
  moveBy(deltaX: number, deltaZ: number): void {
    ...
    const ground = this._terrain.getHeightAt(
      this._mesh.position.x,
      this._mesh.position.z,
    );
    ...
    if (deltaX !== 0 || deltaZ !== 0) {
      ...
      this._mesh.rotationQuaternion = Quaternion.RotationYawPitchRoll(
        yaw,
        0,
        0,
      );
