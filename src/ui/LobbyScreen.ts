...
export class LobbyScreen {
	...
	mount(): void {
		...
		const offKicked = this.room.onMessage(
		...
		this.offs.push(
			typeof offKicked === 'function' ? offKicked : () => undefined,
		);
	...
	private refresh(): void {
		...
		if (
			state === undefined ||
			typeof state.players?.forEach !== 'function'
		) {
			return;
		}
		...
		state.players.forEach((p) =>
			parts.push(`${p.id}:${p.name}:${p.ready ? 1 : 0}`),
		);
	...
	private buildCard(): Control {
		...
		state.players.forEach((p) =>
			players.push({ id: p.id, name: p.name, ready: p.ready }),
		);
		...
		gap(body, 2);
		const modeLabel =
			state.mode === RoomMode.Private ? 'Private' : 'Public';
		...
		body.addControl(
			subtitle(
				`${readyCount} / ${players.length} ready — game starts when everyone is ready.`,
			),
		);
		gap(body, 8);
		...
		readyBtn.onPointerClickObservable.add(() =>
			this.room.send(ClientMessage.ToggleReady),
		);
